...
function displayForumMessages(messages) {
    ...
    const messagesHTML = sortedMessages.map(message => {
        ...
        const userInitials = message.username.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
        return `
            <div class="post-card ${isCurrentUser ? 'own-post' : ''}">
...
function handleNewMessage(newMessage) {
    ...
    const userInitials = newMessage.username.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
    const messageElement = document.createElement('div');
    messageElement.className = `post-card ${isCurrentUser ? 'own-post' : ''}`;
    messageElement.innerHTML = `
        <div class="post-header">
            ...
            <div class="post-info">
                ...
                <div class="timestamp">${formatTimestamp(newMessage.created_at)}</div>
            </div>
        </div>
        <div class="post-content">
            ${newMessage.message}
